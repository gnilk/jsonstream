...
DLL_EXPORT int test_jsdecode_intarray(ITesting *t) {
    ...
    MyIntArray myArray;
    Memfile mf(data);
    JSONDecoderNew jsNew(&mf,&myArray);
...
public:
    ...
    bool SetField(std::string &name, std::string &value) override;
    // Called to get an instance to a sub-object in your class
    IUnmarshal *GetUnmarshalForField(std::string &name) {  return nullptr; }
    // Called to push things to an array you might have
    bool PushToArray(std::string &name, IUnmarshal *pData) {  return false; }
private:
    int numValue;
